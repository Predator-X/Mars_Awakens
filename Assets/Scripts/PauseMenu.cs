...
public class PauseMenu : MonoBehaviour
{
    ...
    //Added for elevator opening mechanizm |
    //                                     V
    public static string elevatorButonNameToLocate=null;
    ...
    [SerializeField] public GameObject lookAt1, lookAt2;
    [SerializeField] GameObject backButtonGameObject, quitButtonGameObject, multiplayerCampainButton;
    public GameObject numberOfPlayersTMP, startButtonMainMenu, mainMenuButtonInPauseMenu;
        ...
        //if (Input.GetKeyDown(KeyCode.Escape)) <---- this before new Input System also work done in PlayerController  if coming back you can delete stuff there  as well
         ...
         if(pauseTheGamePressed)
         {
             ...
             else if(GameIsPaused)
             {
                ...
                EventSystem.current.SetSelectedGameObject(mainMenuButtonInPauseMenu);//<-- this changes button in orther to controll in gamePad 
  ...
  public void Pause()
    ...
    public void SetMainMenuON()
    {
        ...
        lookAt1 = GameObject.Find("CinemachineLookAt");//<--- form here down in this method added while changing ui design
        lookAt2 = GameObject.Find("CinemachineLookAt2");

        //Reset up Buttons 
        backButtonGameObject.SetActive(false);
        numberOfPlayersTMP.SetActive(false);
        multiplayerCampainButton.SetActive(true);
        quitButtonGameObject.SetActive(true);
       // EventSystem.current.SetSelectedGameObject(startButtonMainMenu);< ----------------THIS CANCLED AT THE END OF PRODUCTION 08 / 09 / 2023
    }
    ...
    public virtual void SavePlayer()
    {
       ...
       // SaveSystem.buttonHolder.SetActive(true);<----------------THIS CANCLED AT THE END OF PRODUCTION 08/09/2023
    ...
    public void SplitScreenMultiplayerManu()
    {
        ...
        if (lookAt1 == null)
        {
            ...
            lookAt1 = GameObject.FindGameObjectWithTag("ResetMainMenu").GetComponent<ResetCinemachineForMainMenu>().cinemachine1;
        }
        if (lookAt2 == null) { lookAt2 = GameObject.FindGameObjectWithTag("ResetMainMenu").GetComponent<ResetCinemachineForMainMenu>().cinemachine2; }

        lookAt1.gameObject.SetActive(false);
        lookAt2.SetActive(true);

        quitButtonGameObject.SetActive(false);
        multiplayerCampainButton.SetActive(false);
        backButtonGameObject.SetActive(true);

        numberOfPlayersTMP.SetActive(true);

        EventSystem.current.SetSelectedGameObject
               (GameObject.FindGameObjectWithTag("GameManager").GetComponent<PauseMenu>().startButtonMainMenu);
